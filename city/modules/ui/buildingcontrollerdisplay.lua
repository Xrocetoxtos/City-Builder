    ...
    function BCD.setElements()
        ...
        if #BuildingController.buildings < 1 then
            ...
            for index, bt in ipairs(BuildingTypeDatabase) do
                local element = GuiController.addElement(bt.name, "BT", x, y, GuiController.objectSize, GuiController.objectSize, BCD.clickBuildingType, nil, bt, Sprites.ui.building_types.image, bt.quad) -- TODO ook iets om een actief element aan te tonen bij het renderen
            ...
            for index, building in ipairs(BuildingController.buildings) do
                ...
                local element = GuiController.addElement(building.name, "B", x, y, GuiController.objectSize, GuiController.objectSize, BCD.clickBuilding, nil, building) -- TODO ook iets om een actief element aan te tonen bij het renderen
                x= x + GuiController.objectSize + GuiController.objectMargin
                table.insert(BCD.elements, element)
            end
        end
